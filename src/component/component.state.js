...
export const TimeAction = ($q, timeService) => {
  ...
  const getTime = options => {
    return (dispatch, getState) => {
      ...
      if((options && !options.force) && Object.keys(time).length) {
        return $q.when(time)
          .then(() => dispatch({ type: GET_TIME, payload: time }));
      } else {
        return timeService.get()
          .then(data => dispatch({ type: GET_TIME, payload: data }));
      }
    };
  };
  ...
  const updateTime = payload => {
    return dispatch => {
      return timeService.update(payload)
        .then(() => dispatch({ type: UPDATE_TIME, payload }));
    };
  };
